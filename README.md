...
```sh
git clone https://github.com/saurav-k/dotfiles.git
cd dotfiles
```
...
- **Username**: run `./bootstrap.sh` (it detects your macOS username and offers to set it) OR change the single `user = "sauravtrivedi"` line in `flake.nix`.
